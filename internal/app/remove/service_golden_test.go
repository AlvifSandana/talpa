...
func TestRunDryRunGoldenJSON(t *testing.T) {
	...
	osStat = func(name string) (os.FileInfo, error) {
		return fakeFileInfo{name: filepath.Base(name), size: 321, mode: 0o755, mod: time.Now()}, nil
	}
	...
	normalizeRemoveResult(&res, home)
	b, err := json.MarshalIndent(res, "", "  ")
	if err != nil {
		t.Fatal(err)
	}

	want, err := os.ReadFile(filepath.Join("testdata", "remove_dry_run.golden.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func normalizeRemoveResult(res *model.CommandResult, home string) {
